...
function revealInsights(insights: InsightCardType[], progress: number) {
  ...
  return insights.map((insight) => {
    const body = insight.body.slice(0, Math.max(0, remaining));
  ...
  return (
    <Panel className="h-fit">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={handleRefresh} className="rounded-lg border border-line px-3 py-2 text-xs text-muted transition hover:border-brand/40 hover:text-white">
